...
export const appSlice = createSlice({
  ...
  initialState,
  reducers: {
    setAppConfig: (state, action: PayloadAction<APP_STATE | any>) => {
  ...
  selectors: {
    ...
    getMenuById: (state, id) => {
      const findNodeById = (tree: any, id: any) =>
        tree.reduce((found: any, node: any) => {
          if (found) return found;
          if (node.id == id) return node;
          if (node.children) return findNodeById(node.children, id);
          return undefined;
        }, undefined);
      return findNodeById(state.memu.items, id);
    },
