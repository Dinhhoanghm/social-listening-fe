...
interface Props {
  ...
  footer?: any; // Trạng thái ẩn hiện footer
...
export default function LayoutBase(props: Props) {
  ...
  const _heightHeader = useMemo(() => {
    ...
    let footerHeight = 0;
    if (isPageTab) {
      pageTabHeight = PAGE_TAB_HEIGHT;
    }
    if (footer.showFooter) {
      footerHeight = footer.height;
    }

    return headerHeight + pageTabHeight + footerHeight;
  }, [isPageTab, footer]);
  ...
  return (
    <Layout className={styles.container}>
      ...
      >
        ...
        {/* Hiển thị Footer */}
        {footer?.showFooter && (
          <Footer className={styles.footer} style={{ height: footer?.height }}>
            {FooterComponent}
          </Footer>
        )}
      </Layout>
